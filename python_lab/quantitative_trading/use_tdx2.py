...
'''
    module description
    date: 2020/3/24
    教程地址：http://www.360doc.com/showweb/0/0/901461939.aspx
    说明：解析通达信软件下载的交易数据
    使用：python 文件名.py -t 999999 20070101 20070131 表示 用文本文件转化为1分钟5分钟的数据文件，
        代码是上证指数，日期 20070101到20070131 生成的5分钟数据文件为 sh999999.lc5 ，一分钟文件为 sh999999.lc1
    注意：能直接看5分钟的数据，但不能直接看1分钟的，可能的原因是1分钟的数据文件不是sh999999.lc1形式的，有知道的请告知；
        但是可以换个方法看1分钟的图，方法是sh999999.lc1更名为 sh999999.lc5 ，然后在5分钟图下看1分钟的K线了。
'''
__author__ = "Bigcard"
...
basedir = r'D:\Software\Stock\jcb_hlzq'  # 通达信软件安装home目录
...
def lc1tolc5(p_data):
    ...
    for i in p_data:
        ...
        if t == None:
            raise ValueError(f'time out of range: %s' % i[1])
        lend = len(data)
        j = lend - 1
        while j >= 0:
            if data[j][1] == t: break
...
def convert(p_stkid, p_type='txt', filterfunc=None):
    if p_type == 'txt':  # txt file
        ...
        for fname in txtfiles:
            sys.stderr.write('%s\n' % fname)
            try:
                doc_lines = open(fname).readlines()
            except IOError as e:
...
if __name__ == '__main__':
    ...
    if len(args) < 1:
        print
        'You must specified the stock No.!'
        usage(sys.argv[0])
        sys.exit(0)
