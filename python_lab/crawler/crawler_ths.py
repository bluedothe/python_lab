...
'''
    module description
    date: 2020/7/15
    使用selenium爬取同花顺网站的财务分析数据
    https://blog.csdn.net/CY19980216/article/details/86647597?utm_medium=distribute.pc_aggpage_search_result.none-task-blog-2~all~first_rank_v2~rank_v25-7-86647597.nonecase
'''
...
class StraightFlush():
    ...
    def parse_money_flow_stock(self, ):  # 获取A股个股资金流动
        ...
        flag = False
        with open(r"{}\{}\money_flow_stock_{}.csv".format(self.workSpace + '\\file_down', 'ths_file', self.date), "a") as f:
            for th in ths:
    ...
    def parse_money_flow_concept(self, ):  # 获取A股概念板块资金流动
        ...
        """ 将<th>标签中的string作为DataFrame的header """
        flag = False
        with open(r"{}\{}\money_flow_concept_{}.csv".format(self.workSpace + '\\file_down', 'ths_file', self.date), "a") as f:
            for th in ths:
                aLabel = th.find_all("a")
    ...
    def parse_money_flow_industry(self, ):  # 获取A股概念板块资金流动
        ...
        """ 将<th>标签中的string作为DataFrame的header """
        flag = False
        with open(r"{}\{}\money_flow_industry_{}.csv".format(self.workSpace + '\\file_down', 'ths_file', self.date), "a") as f:
            for th in ths:
                aLabel = th.find_all("a")
                if len(aLabel):  # 概念板块与行业板块读取表头的代码如此笨拙因为与个股的方法在这里竟然只能拿到None,而且两者格式一模一样,让我很奇怪
                    tag = str(aLabel[0])
