...
'''
    ...
    教程地址：https://blog.csdn.net/yanbober/article/details/73822475?depth_1-utm_source=distribute.pc_relevant.none-task&utm_source=distribute.pc_relevant.none-task
    案例地址：www.toutiao.com
    该案例没有调试通过
'''
__author__ = "Bigcard"
...
class CrawlOptAnalysis(object):
    ...
    def _crawl_data(self, offset):
        ...
        url = 'https://www.toutiao.com/api/search/content/?aid=24&app_name=web_search&offset={0}&format=json&keyword={1}&autoload=true&count=20&en_qc=1&cur_tab=1&from=search_tab&pd=synthesis&timestamp=1583666525111'.format(offset, urllib.parse.quote(self.search_word))
        print(url)
        try:
    ...
    def _save_picture(self, page_title, url):
        '''
        把爬取的所有大图下载下来
        下载目录为./toutiao_file/search_word/page_title/image_file
        '''
        ...
        save_dir = os.getcwd() + "/toutiao_file/" + '{0}/{1}/'.format(self.search_word, page_title)
